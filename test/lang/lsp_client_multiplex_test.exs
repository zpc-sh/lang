defmodule Lang.LSPClientMultiplexTest do
  ...
  test "handles concurrent inflight requests over single worker", %{port: port} do
    {:ok, pid} = Lang.LSP.ClientWorker.start_link(host: ~c"127.0.0.1", port: port, root_path: System.cwd!(), max_inflight: 16)

    reqs = for i <- 1..10 do
      delay = 10 * rem(i, 3)
      Task.async(fn ->
        # GenServer.call through worker call API
        Lang.LSP.ClientWorker.call(pid, "test.echo", %{"seq" => i, "delay_ms" => delay}, timeout: 1_000)
      end)
    end
  ...
  test "returns backpressure when inflight exceeds max", %{port: port} do
    {:ok, pid} = Lang.LSP.ClientWorker.start_link(host: ~c"127.0.0.1", port: port, root_path: System.cwd!(), max_inflight: 2)

    # Start two long-running calls to occupy inflight slots
    t1 = Task.async(fn -> Lang.LSP.ClientWorker.call(pid, "test.echo", %{"seq" => 1, "delay_ms" => 300}, timeout: 1_000) end)
    t2 = Task.async(fn -> Lang.LSP.ClientWorker.call(pid, "test.echo", %{"seq" => 2, "delay_ms" => 300}, timeout: 1_000) end)

    # Third call should hit backpressure immediately
    resp = Lang.LSP.ClientWorker.call(pid, "test.echo", %{"seq" => 3, "delay_ms" => 0}, timeout: 1_000)
