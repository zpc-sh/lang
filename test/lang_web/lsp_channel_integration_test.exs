defmodule LangWeb.LspChannelIntegrationTest do
  ...
  test "connects with test_bypass and handles rpc.initialize and fs.preview" do
    ...
    # Initialize
    ref = push(socket, "json", %{"jsonrpc" => "2.0", "id" => "1", "method" => "rpc.initialize", "params" => %{"client" => %{"name" => "itest"}}})
    ...
    ref2 = push(socket, "json", %{"jsonrpc" => "2.0", "id" => "2", "method" => "lang.fs.preview", "params" => %{"path" => path, "max_lines" => 1}})
  ...
  test "fs.search over channel returns results" do
    ...
    # Prepare a temp dir
    root = Path.join(System.tmp_dir!(), "chan_fs_search_" <> Integer.to_string(:erlang.unique_integer([:positive])))
    File.mkdir_p!(root)
    File.write!(Path.join(root, "x.txt"), "TODO: channel test\n")

    ref = push(socket, "json", %{"jsonrpc" => "2.0", "id" => "3", "method" => "lang.fs.search", "params" => %{"root_path" => root, "pattern" => "TODO"}})
    assert_reply(ref, :ok, reply)
  ...
  test "fs.search_code over channel returns matches list" do
    ...
    root = Path.join(System.tmp_dir!(), "chan_fs_search_code_" <> Integer.to_string(:erlang.unique_integer([:positive])))
    File.mkdir_p!(root)
    File.write!(Path.join(root, "sample.js"), "function greet() { return 'hi' }\n")
    patt = "(function_declaration name: (identifier) @function)"

    ref = push(socket, "json", %{
      "jsonrpc" => "2.0",
      "id" => "4",
      "method" => "lang.fs.search_code",
      "params" => %{"root_path" => root, "language" => "javascript", "pattern" => patt, "max_results" => 5}
    })
    assert_reply(ref, :ok, reply)
  ...
  test "fs.scan over channel returns tree and stats" do
    ...
    # Prepare a temp tree
    root = Path.join(System.tmp_dir!(), "chan_fs_scan_" <> Integer.to_string(:erlang.unique_integer([:positive])))
    ...
    ref = push(socket, "json", %{"jsonrpc" => "2.0", "id" => "5", "method" => "lang.fs.scan", "params" => params})
    assert_reply(ref, :ok, reply)
  ...
  test "fs.scan globs and excludes over channel are enforced" do
    ...
    # Prepare a temp tree
    root = Path.join(System.tmp_dir!(), "chan_fs_scan_globs_" <> Integer.to_string(:erlang.unique_integer([:positive])))
    ...
    ref = push(socket, "json", %{"jsonrpc" => "2.0", "id" => "6", "method" => "lang.fs.scan", "params" => params})
    assert_reply(ref, :ok, reply)
