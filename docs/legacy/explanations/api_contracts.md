...
Domains registered with the `Lang.Explanations.Router` must implement the following callbacks:
...
```elixir
Lang.Explanations.Router.route(input, context, opts \\ []) :: {:ok, result} | {:continue, reason} | {:error, reason}
```
