defmodule Lang.Analysis do
  ...
  def update_analysis_session_status(run, status, attrs \\ %{}), do: Run.update_status(run, %{status: status, metadata: attrs[:metadata], error_message: attrs[:error_message]})
  ...
  def fail_analysis_session(run, msg, metadata \\ %{}), do: Run.fail(run, %{error_message: msg, metadata: metadata})
  ...
  def update_analyzed_file_status(file, status, _attrs \\ %{}), do: File.update_status(file, %{status: status})
  def complete_analyzed_file(file, result, ms), do: File.complete(file, %{analysis_result: result}, %{processing_time_ms: ms})
  ...
  def update_analyzed_file(file_id, attrs) when is_binary(file_id), do: with {:ok, file} <- File.by_id(file_id), do: update_analyzed_file(file, attrs)
  ...
  def update_violation_status(v, status, attrs \\ %{}), do: Violation.update_status(v, %{}, %{status: status, metadata: attrs[:metadata]})
  def resolve_violation(v, who, note \\ nil), do: Violation.resolve(v, %{}, %{resolved_by: who, resolution_note: note})
  def acknowledge_violation(v, who, note \\ nil), do: Violation.acknowledge(v, %{}, %{acknowledged_by: who, note: note})
  def suppress_violation(v, who, reason), do: Violation.suppress(v, %{}, %{suppressed_by: who, reason: reason})
  def mark_false_positive(v, who, reason), do: Violation.mark_false_positive(v, %{}, %{marked_by: who, reason: reason})
