  ...
  def perform(%Oban.Job{args: %{"action" => action} = args}) do
    ...
    :telemetry.span([:lang, :agent_task], measurements, metadata, fn ->
      case validate_args(action, args) do
        :ok ->
          try do
            ...
            case handle_action(action, args) do
              :ok ->
                :telemetry.execute([:lang, :agent_task, :result], %{count: 1}, Map.put(metadata, :outcome, :ok))
                {:ok, Map.put(metadata, :status, :ok)}
              {:error, reason} ->
                :telemetry.execute([:lang, :agent_task, :result], %{count: 1}, Map.put(metadata, :outcome, :error))
        ...
        {:error, reason} ->
          Logger.warning("AgentTaskWorker validation failed", action: action, reason: inspect(reason))
          :telemetry.execute([:lang, :agent_task, :result], %{count: 1}, Map.put(metadata, :outcome, :invalid))
      ...
      "trust_level" ->
        with :ok <- require_keys(params, ["agent_id", "threshold"]) do
          case params["threshold"] do
            t when is_number(t) and t >= 0 and t <= 1 -> :ok
            t when is_binary(t) ->
              case Float.parse(t) do
                {v, _} when v >= 0 and v <= 1 -> :ok
                _ -> {:error, {:invalid_threshold, t}}
              end
            other -> {:error, {:invalid_threshold, other}}
          end
        end
      "limit_resources" ->
      ...
      "monitor_performance" ->
        with :ok <- require_keys(params, ["agent_id"]) do
          case {params["duration_ms"], params["window"]} do
            {d, _} when is_integer(d) and d > 0 -> :ok
            {d, _} when is_binary(d) ->
              case Integer.parse(d) do
                {i, ""} when i > 0 -> :ok
                _ -> {:error, :invalid_duration}
              end
            {_, w} when is_binary(w) and w != "" -> :ok
            {_, w} when is_integer(w) and w > 0 -> :ok
            _ -> {:error, :require_duration_or_window}
          end
        end
      _ -> :ok
    end
