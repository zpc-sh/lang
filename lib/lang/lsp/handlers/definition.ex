defmodule Lang.LSP.Handlers.Definition do
  @moduledoc """
  ...
  def handle(id, %{"textDocument" => %{"uri" => uri}, "position" => position}, state) do
    case Map.get(state.documents, uri) do
      ...
      document ->
        ...
        locations = 
          case SymbolAnalyzer.find_definition(word, uri, state.root_uri) do
            {:ok, definitions} ->
  ...
  defp get_word_at_position(text, %{"line" => line, "character" => character}) do
    ...
    word_before = Regex.run(~r/^\w+/, before) |> List.first("") |> String.reverse()
    word_after = Regex.run(~r/^\w+/, after_cursor) |> List.first("")
