defmodule Lang.LSP.SecurityMiddleware do
  ...
  # Ensure caller's org can make a request (cheap per-call guard; upstream may enforce stronger quotas)
  defp allow_billing(%{"params" => params}) when is_map(params) do
    org_id =
      get_in(params, ["current_org", "id"]) ||
    ...
    case org_id do
      ...
      id when is_binary(id) ->
        case Lang.Billing.can_make_request?(id) do
          {:ok, :allowed} -> :ok
          {:error, :limit_exceeded} -> {:error, {:billing_blocked, :limit_exceeded}}
          other ->
            # Fail open but log unexpected responses to avoid availability impact
            Logger.warn("Billing gate unexpected response", org_id: id, result: inspect(other))
            :ok
        end
      _ -> :ok
    end
