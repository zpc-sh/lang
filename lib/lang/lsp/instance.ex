defmodule Lang.LSP.Instance do
  ...
  def handle_call({:handle_json, json}, _from, state) do
    {reply, new_state} =
      with {:ok, msg} <- Jason.decode(json),
        ...
        # Capture workspace root on initialize; renew lease on any activity
        updated_state = maybe_capture_workspace_root(sanitized_msg, state)
        next_state = renew_lease(updated_state)
        reply =
        ...
        {:error, err} -> {Jason.encode!(%{"jsonrpc" => "2.0", "id" => get_in_safe(json, ["id"]) || 0, "error" => %{code: -32003, message: to_string(err)}}), state}
        _ -> {nil, state}
      end
  ...
  defp maybe_capture_workspace_root(%{"method" => method, "params" => %{} = params}, state) when is_binary(method) do
    if method == "rpc.initialize" do
      ws_root =
        params["workspace_root"] ||
          normalize_root(params["rootUri"], params["rootPath"]) ||
          state.workspace_root

      if is_binary(ws_root) and ws_root != state.workspace_root do
        %{state | workspace_root: ws_root}
      else
        state
      end
    else
  ...
  defp maybe_capture_workspace_root(_msg, state), do: state

  defp normalize_root(root_uri, root_path) do
    cond do
      is_binary(root_uri) and String.starts_with?(root_uri, "file://") ->
        <<"file://", rest::binary>> = root_uri
        safe_expand("/" <> String.trim_leading(rest, "/"))
      is_binary(root_path) -> safe_expand(root_path)
      true -> nil
    end
  end
  ...
  defp inject_identity(%{"params" => params} = msg, state = %{cid: cid, org: org}) do
    new_params =
      params
      |> Map.put_new("current_cid", cid)
      |> Map.put_new("current_org", org)
      |> maybe_put_workspace_root(state)

    Map.put(msg, "params", new_params)
  end
  defp inject_identity(msg, _), do: msg

  defp maybe_put_workspace_root(params, %{workspace_root: root}) do
    cond do
      is_binary(Map.get(params, "workspace_root")) -> params
      is_binary(root) and root != "" -> Map.put(params, "workspace_root", root)
      true -> params
    end
  end
