defmodule Lang.Providers.Gemini do
  ...
  defp generate_content(prompt, opts \\ []) do
    ...
    case make_request("/models/#{model}:generateContent", payload) do
      {:ok, response} ->
  ...
  defp get_api_key do
    case Application.get_env(:lang, :ai_providers)[:gemini_api_key] do
      nil ->
        raise "GEMINI_API_KEY not configured in :lang, :ai_providers"
