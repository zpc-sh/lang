defmodule Lang.Providers.XAI do
  ...
  @impl Lang.Providers.Provider
  def available? do
    case Lang.Providers.Credentials.resolve_api_key(:xai, %{}) do
      {:ok, _} -> true
      _ -> false
    end
  end
  ...
  def command_mission(request, opts \\ []) do
    ...
    case make_request("/chat/completions", payload, opts) do
      {:ok, response} ->
  ...
  defp get_api_key(opts \\ []) do
    case Lang.Providers.Credentials.resolve_api_key(:xai, opts) do
      {:ok, key} -> key
      {:error, _} -> raise "XAI_API_KEY not configured for this request"
    end
  end
