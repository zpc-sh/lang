defmodule LangWeb.LSPMonitorLive do
  ...
  def handle_event(
        "start_stream",
        %{"analysis" => %{"content" => content, "format" => format}},
        socket
      ) do
    case Lang.LSP.Client.request("lang.analyze.stream", %{content: content, format: format}) do
      {:ok, %{"stream_id" => stream_id}} ->
        if connected?(socket),
          do: Phoenix.PubSub.subscribe(Lang.PubSub, "lsp:analysis:#{stream_id}")
  ...
  def handle_info({:started, data}, socket) do
    {:noreply,
     update(socket, :analysis_events, fn ev -> cap_events(ev ++ ["started: #{inspect(data)}"]) end)}
  end
  ...
  def handle_info({:progress, data}, socket) do
    ...
    {:noreply,
     update(socket, :analysis_events, fn ev -> cap_events(ev ++ ["progress: #{pct}%"]) end)}
  end
  ...
  def handle_info({:error, data}, socket) do
    {:noreply,
     update(socket, :analysis_events, fn ev -> cap_events(ev ++ ["error: #{inspect(data)}"]) end)}
  end
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <%= if @lsp_caps do %>
              <div id="lsp-caps" class="mt-4 p-3 rounded border border-gray-200 bg-gray-50">
                <div class="text-sm text-gray-700">
                  Methods available: {(@lsp_caps["methods"] || []) |> Enum.count()}
                </div>
              </div>
            <% end %>
            <%= if @lsp_methods != [] do %>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
          <div class="bg-white shadow rounded-lg p-6">
            ...
            <div
              id="analysis-events"
              class="mt-4 max-h-40 overflow-auto text-sm font-mono bg-gray-50 p-2 rounded border border-gray-200"
            >
              ...
              <div class="grid grid-cols-1 md:grid-cols-6 gap-3 items-end">
                ...
                <div class="md:col-span-1">
                  <button
                    id="scan"
                    type="submit"
                    class="w-full px-3 py-2 bg-emerald-600 text-white rounded-md hover:bg-emerald-700"
                  >
                    Scan
                  </button>
                </div>
            ...
            <div class="mt-4 text-sm">
              ...
              <%= if @scan_result do %>
                <pre
                  ...
                  class="p-2 bg-gray-50 rounded border border-gray-200 overflow-auto"
                >{inspect(@scan_result, pretty: true, limit: 50)}</pre>
              <% else %>
